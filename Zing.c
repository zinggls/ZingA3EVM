...
CyU3PReturnStatus_t Zing_RegWrite(uint16_t addr, uint8_t* buf, uint16_t len)
{
	...
	apiRetStatus = Zing_Transfer_Send2(&dmaControlOut,glZingControlOutBuffer,len+ZING_HDR_SIZE);
...
CyU3PReturnStatus_t Zing_RegRead(uint16_t addr, uint8_t* buf, uint16_t len)
{
    ...
    status = Zing_Transfer_Send2(&dmaControlOut,glZingControlOutBuffer,ZING_HDR_SIZE);
...
CyU3PReturnStatus_t Zing_DataWrite(uint8_t* buf, uint32_t len)
{
	...
	apiRetStatus = Zing_Transfer_Send2(&dmaDataOut,glZingDataOutBuffer,len);
#if DBG_LEVEL >= DBG_TYPE_ZING_TR
	if(apiRetStatus==CY_U3P_SUCCESS) {
		{
			int i;
...
CyU3PReturnStatus_t Zing_DataWrite2(uint8_t* buf, uint32_t len)
{
	...
	apiRetStatus = Zing_Transfer_Send2(&dmaDataOut,buf,len);
...
CyU3PReturnStatus_t Zing_DataRead(uint8_t* buf, uint32_t* len_pt)
{
	...
	apiRetStatus = Zing_Transfer_Recv2(&dmaDataIn,glZingDataInBuffer,len_pt);
...
void Zing_Test_DataTx2 (uint32_t repeat, uint32_t length, uint32_t pattern)
{
    ...
    CyU3PDmaBuffer_t Buf;
    CyU3PDmaChannel* dma_ch = &dmaDataOut;
...
{
	CyU3PReturnStatus_t status = CY_U3P_SUCCESS;
	CyU3PDmaChannel* dma_ch = &dmaDataIn;
	CyU3PDmaBuffer_t Buf;
    ...
    )
{
	...
	status = Zing_Transfer_Send2(&dmaControlOut,glZingControlOutBuffer,length+ZING_HDR_SIZE);
